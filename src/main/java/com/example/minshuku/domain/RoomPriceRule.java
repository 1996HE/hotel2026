...
import java.time.LocalDate; // 読み込み日付范围フィールド使用の本地日付型。
...
public class RoomPriceRule { // 定義部屋时令料金ルールエンティティ，対応 room_price_rules テーブル。
  ...
  private Boolean active; // 標记该料金ルール是否启用。
  ...
  public Boolean getActive() { return active; } // 返却料金ルール是否启用。
  public void setActive(Boolean active) { this.active = active; } // 設定料金ルール是否启用。
