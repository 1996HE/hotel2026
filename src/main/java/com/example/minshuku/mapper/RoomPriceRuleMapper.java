...
public interface RoomPriceRuleMapper { // 定義 room_price_rules テーブルのデータ访问インターフェース。
  List<RoomPriceRule> findAllWithRoom(); // 検索料金ルールと結合带出部屋信息。
  RoomPriceRule findBestRule(@Param("roomId") Integer roomId, @Param("stayDate") LocalDate stayDate); // 検索某部屋某日付命中の最高优先级料金ルール。
  int insert(RoomPriceRule rule); // 新規登録料金ルール记录。
  int delete(@Param("id") Integer id); // 削除单条料金ルール记录。
  int deleteByIds(@Param("ids") List<Integer> ids); // 一括削除选中の料金ルール记录。
  int countOverlapping(@Param("roomId") Integer roomId, @Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate); // 集計同部屋日付重叠の料金ルール数量。
}
